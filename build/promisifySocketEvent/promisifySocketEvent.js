...
const promisifySocketEvent = (userCallback, convertResult, convertError) => {
  ...
  return (eventData = undefined, eventCb = undefined) => {
    ...
    const socketioCallback = typeof eventData === 'function' ? eventData : eventCb && typeof eventCb === 'function' ? eventCb : undefined;
    return Promise.resolve().then(() => userCallback(cleanData)).then(success => socketioCallback(null, convertResult ? convertResult(success) : success)).catch(error => socketioCallback(convertError ? convertError(error) : error));
  };
};

var _default = promisifySocketEvent;
